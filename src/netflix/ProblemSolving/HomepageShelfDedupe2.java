...
================================================================================
*/
public class HomepageShelfDedupe2 {

    private final int x;
    private final List<List<Integer>> kept = new ArrayList<>();
    private final Set<Integer> globalSeen = new HashSet<>();

    public HomepageShelfDedupe2(int x) {
        if (x < 0) throw new IllegalArgumentException("x must be >= 0");
        this.x = x;
    }

    /** Build the homepage from scratch — replaces any prior state. */
    public void initial(int[][] shelves) {
        kept.clear();
        globalSeen.clear();
        for (int[] shelf : shelves) append(shelf);
    }

    public void append(int[] shelf) {
        kept.add(dedupeShelf(shelf));
    }
    ...
    private List<Integer> dedupeShelf(int[] shelf) {
        List<Integer> rowOut = new ArrayList<>();
        Set<Integer> localSeen = new HashSet<>();
        for (int t : shelf) {
            if (rowOut.size() < x) {
                if (globalSeen.add(t)) { rowOut.add(t); localSeen.add(t); }
            } else if (localSeen.add(t)) {
                rowOut.add(t);
            }
        }
        return rowOut;
