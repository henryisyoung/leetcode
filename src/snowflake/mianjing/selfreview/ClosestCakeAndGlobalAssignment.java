...
public class ClosestCakeAndGlobalAssignment {
    ...
    public int nearestCake(int[] A, int start) {
        int n = A.length;
        for (int d = 0; d < n; d++) {
            int left = start - d, right = start + d;
            if (left >= 0 && A[left] == 1) return d;
            if (right < n && A[right] == 1) return d;
        }
        return -1;
    }
     ...
     * ------------------------------------------------------------------- */
    public int GlobalAssignmentMinDist(int[] line) {
        if (line == null) {
            return 0;
        }

        int length = line.length;
        List<Integer> people = new ArrayList<>(), cakes = new ArrayList<>();
        ...
        if (n < m) {
            throw new IllegalStateException("Not enough cakes");
        }
        int[][] dp = new int[m + 1][n + 1];
        for (int[] row : dp) Arrays.fill(row, length * length);
    ...
    public int[] GlobalAssignmentMinDistAssignment(int[] line) {
        if (line == null) {
            return new int[0];
        }
        ...
        while (i > 0) {
            if (dp[i][j] == dp[i][j - 1]) {
                j--;
            } else if ( dp[i - 1][j - 1] + Math.abs(people.get(i - 1) - cakes.get(j - 1)) == dp[i][j]) {
                result[i - 1] = cakes.get(j - 1);
                i--;
                j--;
            }
        }
